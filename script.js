document.addEventListener('DOMContentLoaded', () => {
    ...
    const bubbleBackground = document.querySelector('.bubble-background'); // Bubble container
    ...
    const handleFormSubmit = async (event) => {
        ...
        try {
            // --- IMPORTANT: Replace YOUR_FORMSPREE_ID in the HTML action attribute ---
    ...
    function createBubbles() {
        if (!bubbleBackground) return; // Exit if container not found

        // Determine bubble count based on screen width (density adjustment)
        const screenWidth = window.innerWidth;
        let bubbleCount = 25; // Default for larger screens
        if (screenWidth < 768) {
            bubbleCount = 15; // Fewer bubbles on mobile
        } else if (screenWidth < 1200) {
            bubbleCount = 20; // Medium count for tablets/small laptops
        }
        ...
        for (let i = 0; i < bubbleCount; i++) {
            ...
            const leftPosition = Math.random() * 100; // Horizontal position (0-100%)
            const drift = (Math.random() - 0.5) * 10; // Horizontal drift (-5vw to +5vw)

            // Set CSS variables for this specific bubble
